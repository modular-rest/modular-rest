...
class DirectoryWalker {
            ...
            list.forEach(function(file)
            {
                ...
                fs.stat(file, function(err, stat)
                {
                    ...
                    if (stat && stat.isDirectory()) 
                    {
                        ...
                        //results.push(file);
                        this.walk(file, settings, function(err, res){
    ...
    find(dir, settings) {
        ...
        {
            this.walk(dir, settings, (err, result) => {
                if(err) reject(err);
                else don(result);
            });
        });
...
DirectoryWalker.instance = new DirectoryWalker();
module.exports = DirectoryWalker.instance;